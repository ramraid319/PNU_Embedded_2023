...
void DMA1_Channel1_IRQHandler() {
  if (DMA_GetITStatus(DMA1_IT_TC1) != RESET) {
    ...
    if (gameStatus == 0 && ADC_Value[0] <= 10)
    {
        coinCount++;
        LCD_ShowString(20, 240, "Coin In : ", BLACK, WHITE);
        LCD_ShowNum(80, 240, (uint32_t)coinCount, 4, BLACK, WHITE);
    }
    ...
    if (gameStatus != 3 && LED[0] == 1 && LED[1] == 1)
    {
      
        end_time = TIM3->CNT;
        result_time = end_time - start_time;
        LCD_ShowString(20, 280, "!!!!!!!Game Win!!!!!!!", BLACK, WHITE);
        gameStatus = 3;
    }
...
{  
  ...
  while (1) {
    ...
    else if (gameStatus == 2)
    {
      ...
      result_time = (start_time - end_time) / CLOCKS_PER_SEC;
    }

    else if (gameStatus == 3)
    {
      LCD_ShowNum(20, 40, (uint32_t)result_time, 4, BLACK, WHITE);
